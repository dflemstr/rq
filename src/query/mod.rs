...
mod test {
    ...
    fn parse_kitchen_sink() {
        let expected = Query(vec![
            Process(
                "dostuff".to_owned(),
                vec![
                    Expression::Value(value::Value::String("foo".to_owned())),
                    Expression::Function(vec!["x".to_owned()], "x+3".to_owned()),
                    Expression::Function(
            ...
            Process(
                "other".to_owned(),
                vec![
                    Expression::Value(value::Value::String("xyz".to_owned())),
                    Expression::Value(value::Value::from_f64(2.0)),
                ],
            ),
            Process("bar".to_owned(), vec![]),
        ]);
        let actual = Query::parse(
            "dostuff foo (x)=>{x+3} (a, b, c) => {a + b - c} | other xyz 2 \
             | bar",
        )
        .unwrap();
    ...
    fn parse_process_one_arg() {
        let expected = Query(vec![Process(
            "select".to_owned(),
    ...
    fn parse_process_one_arg_ident_numbers() {
        let expected = Query(vec![Process(
            "select".to_owned(),
            vec![Expression::Value(value::Value::String("abc123".to_owned()))],
        )]);
    ...
    fn parse_process_one_arg_underscore() {
        let expected = Query(vec![Process(
            "select".to_owned(),
            vec![Expression::Value(value::Value::String(
                "abc_def".to_owned(),
            ))],
        )]);
